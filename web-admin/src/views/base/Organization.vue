      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'action'">
            <a-space>
              <a-button type="link" size="small" @click="editOrganization(record)">
                <edit-outlined /> 编辑
              </a-button>
              <a-popconfirm
                title="确定要删除该组织机构吗？"
...
const columns = [
  ...
  { title: '组织机构名称', dataIndex: 'name', key: 'name' },
...
const loadOrganizations = async () => {
  try {
    ...
    organizations.value = [
      { id: 1, name: '集团总部', parentId: null, level: 1, status: 1 },
      { id: 2, name: '技术研发中心', parentId: 1, level: 2, status: 1 },
      { id: 3, name: '市场营销中心', parentId: 1, level: 2, status: 1 },
      { id: 4, name: '财务管理中心', parentId: 1, level: 2, status: 1 },
      { id: 7, name: '前端开发部', parentId: 2, level: 3, status: 1 },
      { id: 8, name: '后端开发部', parentId: 2, level: 3, status: 1 }
    ]
  }
}
...
const deleteOrganization = async (id) => {
  try {
    await axios.delete(`/api/base/organization/${id}`)
...
const handleOk = async () => {
  try {
    ...
    if (form.id) {
      await axios.put(`/api/base/organization/${form.id}`, form)
      await loadOrganizations()
    } else {
      await axios.post('/api/base/organization', form)
      await loadOrganizations()
    }
